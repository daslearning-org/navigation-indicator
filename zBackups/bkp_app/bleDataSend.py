...
class GattCallback(PythonJavaClass):
    ...
    def onServicesDiscovered(self, gatt, status):
        print("=== SERVICES DISCOVERED ===")
        #print("Device MAC:", gatt.getDevice().getAddress())
        services = gatt.getServices()

        for service in services:
            #s_uuid = service.getUuid().toString()
            #print("Service:", s_uuid)

            characteristics = service.getCharacteristics()
            for char in characteristics:
                #c_uuid = char.getUuid().toString()
                props = char.getProperties()
                if (props & 0x08) or (props & 0x04):
                    print("Writable char found:", char.getUuid().toString())
                    self.ble.rx_char = char
                    self.ble.ready = True
                    print("BLE ready")
...
class BLEClient:
    ...
    def send(self, message):
        ...
        if not self.ready:
            print("BLE not ready yet to send commands!!")
            return
